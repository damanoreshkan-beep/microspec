...
const Disc = ({ playing }) => html`
  <div class="grid place-items-center w-40 sm:w-48 aspect-square">
    <svg viewBox="0 0 100 100" class=${"w-full h-full v2m-disc" + (playing ? " is-playing" : "")} aria-hidden="true">
      <circle cx="50" cy="50" r="48" class="fill-base-300" />
      <circle cx="50" cy="50" r="47" fill="none" class="stroke-base-content/10" stroke-width="0.5" />
      ${[41, 35, 29, 23].map((r) => html`<circle cx="50" cy="50" r=${r} fill="none" class="stroke-base-content/10" stroke-width="0.4" />`)}
      <circle cx="50" cy="50" r="16" class="fill-primary/90" />
      <circle cx="50" cy="50" r="3" class="fill-base-300" />
    </svg>
  </div>`;
