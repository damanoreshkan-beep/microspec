...
## 7. A vertical clip is not a vertical file — the framing (owner, 2026-08-06)

**The report:** "portrait videos should be full screen, we are in portrait mode." **The complication, measured
before any code:** on the sources the owner actually reels, a vertical clip does not arrive as a vertical file.

Measured browser-free (a Deno probe against the live extractor, then `ffprobe`/`ffmpeg` on the files it
returned; the numbers below are from a page whose every result is vertical):

| channel | what it says | what it is |
|---|---|---|
| the grid poster | **323x182** on 98 of 98 items, i.e. exactly 16:9 | useless — one fixed thumbnail size per site |
| the preview file's container | **320x180**, `DAR 16:9`, WebM | a landscape canvas |
| `cropdetect` on the same file | **`crop=100:180:108:0`** | a 100x180 **portrait** frame, pillarboxed |
| the page's own clip (`/feed/stream`) | HLS master `RESOLUTION=1080x1920` | genuinely portrait, at full size |

So: **every metadata channel says 16:9 and only the pixels disagree**, and `object-fit` alone can never fix it
— the bars are pixels in the file. Two surfaces, two mechanisms, one rule:

- **The reel slide** samples a frame into a 64x36 canvas (the same trick the poster filter already uses) and
  asks `packages/runtime/aspect.js` `frameContent()` where the content is. Up to four samples ~340ms apart,
  committing on the first **consecutive pair that agrees** — a dark *scene* is the false positive and bars do
  not move. Structural guards on each sample: the bar run is ≥94% of the column, the two bars match within 2
  cells, the bar is black and the content is ≥18 luma brighter, and nothing survives if the content is a
  sliver. Then `fitPlan()` returns one composited transform over `object-fit: contain`:
  `translate(dx,dy) scale(k)` — the file is laid out whole, and the *content* is zoomed to cover the surface.
  Cached per clip, so a re-mount or a restore never re-measures and never re-frames a clip mid-watch. A clip
  that fills also skips the blurred ambient backdrop: there is nothing behind it to fill.
- **The full clip** needs none of that: `/feed/stream` states the true size, so the runtime `Player` takes an
  opt-in `fill` and switches `object-fit` (not a transform — the native controls are part of the element and
  a scaled element scales them too).

**The budget is what keeps a landscape clip untouched.** `maxCrop = 0.25` of the content's longer axis, and it
is measured against the gate's own breakpoints for a 9:16 content box: **0.2%** at 320x568, **17.9%** at
384x832, **20.0%** at 412x915 — then **41.3%** at split 412x430 and **46.9%** at split-sm 360x340. So it fills
on every phone shape and refuses in split-screen, where cropping half a clip away would be amputation; a 16:9
clip costs **74%** on a phone and is never touched. The surface is measured off the slide (`ResizeObserver`),
never assumed from the viewport, because the answer inverts between those shapes.

**CORS is the enabling condition, and it has a fallback.** A canvas draw of a cross-origin video taints unless
the response carries CORS, so the element now asks for `crossOrigin="anonymous"`. Measured: the sources' CDNs
answer with `access-control-allow-origin` (reflected or `*`); where one does not, the load fails and the
existing `onError` path swaps to the sealed proxy, which always sends it. An inline `data:` clip is skipped —
same rule the poster filter already documents.

**Gates.** `runtime_test.js` ("aspect …", 11 cases) pins the maths on synthetic frames including every
false-positive shape. The browser gate seeds an **inline 1096-byte VP8 clip** carrying 20x36 of content
between black bars — the real geometry at 1/5 scale — as slide 0, and asserts `data-fit="fill"`,
`data-zoom ≈ 3.85`, the computed `transform` matrix on the element, and that no ambient decoder was started.
The gitignored `packages/runtime/aspect.local_test.js` is the ground truth: it pulls today's previews from the
real pages and checks our detector against `ffmpeg cropdetect` axis by axis — last run **3 pillarboxed
vertical + 4 full-frame landscape, agreement on every clip, no false positive**.

**Not settled here:** a pillarboxed preview is 180p, so filling it means upscaling its central ~100px strip
~4.6x — full-screen but soft. The sharp answer is to promote the active slide to the real ladder
(`/feed/stream`), which is a per-slide server round trip and a separate feature; the tap already does it.
