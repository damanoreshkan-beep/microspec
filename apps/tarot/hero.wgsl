...
fn fs(@builtin(position) frag: vec4f) -> @location(0) vec4f {
  ...
  // ink.x is the app's FLARE: the candle leaps when the cards turn and decays over about a second. The
  // stage reacting to the draw is what ties the atmosphere to the ritual instead of looping beside it.
  let flare = clamp(u.ink.x, 0.0, 1.0);

  let centre = vec2f(0.0, -0.10) + sway;
  let d = distance(vec2f(p.x, p.y * 0.86), centre);          // slightly elliptical — a lamp, not a torch
  let pool = pow(smoothstep(0.34 + flare * 0.10, 0.015, d), 1.6) * flick * (1.0 + flare * 0.55);
