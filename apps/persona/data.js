// persona — the shelf. Everything a card needs comes from ONE call to the edge (`characters()`), which the
// runtime module caches for the session; a typed name goes to Wikipedia through the edge and comes back as
// CANDIDATES — the same card shape, flagged so the shelf sections them apart and the detail body knows to
// create the character before talking to them.
//
// Card text is picked in the ACTIVE locale here (the runtime resolves one field per slot, and both languages
// are stored on the row), read off <html lang>, which the runtime keeps current. A locale switch re-labels
// the shelf on the next load, not instantly — the drill-down (view.js) follows `loc` live.
...
// A portrait is the recognition on this shelf; a character without one gets a quiet monogram in one hue near
// the accent, never a random colour (colour means something here).
const monogram = (name) => letterTile(name, { w: 300, h: 450, hue: 42, sat: 14, light: 22, fontSize: 84 });
...
export function toItem(c, loc) {
  ...
  return {
    ...
    url: c.url,
    // section flags — exactly one is true
