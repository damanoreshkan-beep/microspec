...
see the phase table in §8 for what is done. **The shell is verified on a device** (S25, 2026-08-04): bridge-3 actions all green through `os`, including the foreground service and alarms.

**A capability is not reachable until something can ASK for its permission** — `system.grant` (bridge 5). Before it, `cell.info` was granted to the page, compiled into the shell, and refused forever, while its tile showed green and answered taps with "revoke it in settings". `wifi` only worked by coincidence: the geolocation tile happened to request the permission it also needed.

**Deploy checklist, learned twice the hard way:** `edge/apk/*.js` are ALL deployed together (`scp *.js`) — copying only the templates left the generated `capabilities.js` stale and the bridge refused capabilities it had compiled in.
...
| **4** ✅ | Permissions registry + grouped screen + schema enum + uk/en | 3 | **done 2026-08-04** — ONE registry, two backends (`packages/runtime/permissions.js`); rendered as an OS home launcher, since a permission is a thing you own and a grid is the shape an OS already uses for those. State is a dot, never a caption. |
...
| **7** ✅ | `files`, `system` | 3 | **done 2026-08-05** — bridge 20 `files.grant/roots/list/read/write` + a real explorer in `os`, on SAF so the manifest gains NOTHING (`MANAGE_EXTERNAL_STORAGE` hands over the whole device and reads as spyware; the user grants a subtree instead). Bridge 21 adds `system.settings` — the only exit from a permission denied twice, which no dialog can ever re-ask — plus `system.battery` and `files.share` (MediaStore, not a FileProvider). Verified on device at bridge 20: `granted` carries `files`, `missing: —`. |
