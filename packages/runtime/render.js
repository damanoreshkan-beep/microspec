...
function Profile({ tab }) {
  ...
  return html`<div class="flex flex-col gap-3 pt-1">
    ...
    ${!inTelegram() ? html`<a id="p-tg" href=${`https://t.me/dreamstudio_x_bot?startapp=${A.spec.id}`} target="_blank" rel="noopener" class="card sf-raised sf-e2 rounded-[var(--ms-r)] active:scale-[.99] transition"><div class="card-body p-4 flex-row items-center gap-3">${Icon("mdi:telegram", "text-xl text-[#29a9eb]")}<span class="flex-1 min-w-0 truncate font-medium text-left">${sys("openTelegram", loc)}</span>${Icon("lucide:arrow-up-right", "opacity-60")}</div></a>` : null}
