#!/usr/bin/env bash
# Reproducible build of apps/brick/assets/brick.wasm
...
# Build:             bash tools/wasm/brick/build.sh
#
# Unlike tools/wasm/v2m there are no third-party sources to fetch: every line of game.c is ours
# (MIT), which is the whole point — see apps/brick/RESEARCH.md §0 for why no existing Mario
# engine could ship from a public repo.
set -euo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="$HERE/../../../apps/brick/assets/brick.wasm"
mkdir -p "$(dirname "$OUT")"
