...
// bright, slightly inharmonic metallic pluck
...
export function kalimba({ S }) {
  ...
  const pluck = (i) => { const e = ensure(); if (e) e.strike(TINES[i].freq, TIMBRE); flash(i); };
  ...
  const play = (s) => {
    ...
    s.seq.forEach((note, step) => song.current.push(setTimeout(() => {
      if (note != null) { const i = NOTE_IDX[note]; if (e) e.strike(TINES[i].freq, TIMBRE); flash(i); }
  ...
  return html`<div class="flex flex-col items-center gap-5 pt-1">
    <div class="w-full max-w-[440px] rounded-3xl bg-base-300 border border-base-content/10 p-3 pb-2 shadow-inner">
      <div class="flex items-end justify-center gap-[3px] h-[280px]">
        ${TINES.map(({ i, note, letter, tonic, h }) => html`<button data-tine=${i} data-note=${note} aria-label=${note}
          onClick=${() => pluck(i)}
          class=${`tine relative flex-1 min-w-0 rounded-t-md bg-gradient-to-b from-zinc-100 to-zinc-400 shadow-sm touch-manipulation transition-all duration-100 active:translate-y-0.5 ${lit.has(i) ? "ring-2 ring-primary -translate-y-1.5 z-10 brightness-110" : ""}`}
          style=${`height:${h}%`} key=${i}>
          ${tonic ? html`<span class="absolute inset-x-0 top-0 h-1.5 rounded-t-md bg-amber-400"></span>` : null}
