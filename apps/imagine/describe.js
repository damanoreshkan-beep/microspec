      ...
      ${phase === "reading"
        ...
        : html`<${Fragment}>
          ...
          <div class="flex gap-2 pt-1">
            <button data-copy class="btn btn-primary flex-1 rounded-2xl gap-2" onClick=${copy}>${Icon("lucide:copy", "text-lg")}${T(t, "copy")}</button>
            <button data-ask class="btn btn-outline rounded-2xl gap-2 shrink-0" onClick=${askAgain}>${Icon("lucide:message-circle-question", "text-lg")}${T(t, "askMore")}</button>
          </div>
        </${Fragment}>`}
    </div>` : null}
