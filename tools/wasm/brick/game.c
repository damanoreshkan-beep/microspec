...
#define ACC_WALK   16             /* 0.0547 px/f²  */
#define MAX_WALK   450            /* 1.5625 px/f   */
#define MAX_RUN    738            /* 2.5625 px/f   */
#define FRICTION   15             /* 0.0508 px/f²  */
#define SKID       44             /* 0.1523 px/f²  — turning against your own momentum */
#define JUMP_V     (-1152)        /* -4.0 px/f     */
#define GRAV_HOLD  34             /* 0.1172 px/f²  */
#define GRAV_FALL  126            /* 0.4375 px/f²  */
#define MAX_FALL   1296           /* 4.5 px/f      */
#define BOUNCE_V   (-698)         /* the hop after a stomp */
#define WALKER_V   81             /* 0.28 px/f     */
...
static void gen_one(void) {
  ...
  if (seg_type != SEG_GAP && safe_left == 0 && c > 8 && rnd(100) < 4 + d / 24) {
    Ent *e = spawn((d > 140 && (xr() & 3) == 0) ? K_HOPPER : K_WALKER,
                   c * TILE + 1, ground_row * TILE - EH);
    if (e) { e->dir = 0; e->vx = -WALKER_V; }
  }
  gen_col++;
}
...
static void step_enemy(Ent *e) {
  ...
  if (e->vy >= 0) {
    ...
    if (solid_px(px, b) || solid_px(px + EW - 1, b)) {
      ...
      if (e->kind == K_HOPPER && (frame_no & 63) < 2) e->vy = -760;
    }
  }
