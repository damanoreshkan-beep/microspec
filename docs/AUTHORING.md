...
- `/_rt/watch.js` — `<Bell source params app loc/>`: the farm's "tell me when". A TAB opts in with one
  spec key, `tabs[].watch.source`, and the app bar grows a BELL while that tab is open — on the tab, not
  the app, because the globe's ISS screen and its earthquakes screen are two different rules; the EDGE owns the source matrix (its unit,
